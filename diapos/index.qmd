...
-   Miembro de la [**Sociedad Española de Estadística e IO**]{.hl-yellow}, de la [**Real Sociedad Matemática Española**]{.hl-yellow} y de la [**Data Visualization Society**]{.hl-yellow}.
:::
:::
:::
...
-   `{tibble}`: [**optimizando data.frame**]{.hl-yellow}
-   `{tidyr}`: limpieza de datos
-   `{readr}`: [**carga datos rectangulares (.csv)**]{.hl-yellow}
-   `{dplyr}`: gramática para depurar
-   `{stringr}`: manejo de textos
-   `{ggplot2}`: visualización de datos
-   `{tidymodels}`: modelización/predicción
...
También tenemos los paquetes `{purrr}` para el manejo de listas, `{forcast}` para cualitativas, `{lubridate}` para fechas, [**`{readxl}` para importar archivos .xls y .xlsx**]{.hl-yellow}, `{rvest}` para web scraping y [**`{rmarkdown}` para comunicar resultados**]{.hl-yellow}.
...
* Investiga las opciones del paquete `{DT}` (con la función `datatable()`) que nos permite [**introducir dentro de los `.qmd` los datos en formato de tabla dinámica**]{.hl-yellow}, permitiendo ordenar y filtrar.
