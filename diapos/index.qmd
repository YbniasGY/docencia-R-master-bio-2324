...
[**Cruzando datos. Bucles. Mapeando listas. Depurando datos**]{style="color:#444442;"}
...
[**¿Cómo depurar/explorar nuestros datos antes de modelizar?**]{style="color:#444442;"}
...
* [**Tipología**]{.hl-yellow} de las variables. ¿Todas mis variables predictoras son numéricas o debo? ¿Mi variable objetivo es categórica?
...
* [**Atípicos y ausentes**]{.hl-yellow}. ¿Tengo valores atípicos (outliers)? En caso afirmativo, ¿cómo tratarlos? Tras tratar atípicos, ¿tengo datos ausentes?
...
* [**Variables dummy**]{.hl-yellow}. ¿Debo recategorizar variables que no sean numéricas? 
...
El objetivo será [**predecir el peso**]{.hl-yellow} (variable objetivo continua) mediante una regresión.

---

## Depuración

Para simplificarlo vamos primero a [**seleccionar las columnas**]{.hl-yellow} de estatura, peso, edad y sexo.
...
En este caso [**todas tienen rangos permitidos**]{.hl-green}
...
Tras marcarlos como ausente tendremos dos opciones: 

* [**eliminar dichas observaciones**]{.hl-yellow} con `drop_na()` de nuevo (problema: eliminas TODA la fila)

* [**imputar por la media**]{.hl-yellow} (sin contar con los ausentes)

. . .

Para lo segundo podemos usar el paquete `{mice}` (ver <https://amices.org/mice/>) destinado a [**imputar ausentes**]{.hl-yellow}
...
Con la función `md.pattern()` (de dicho paquete) podemos [**visualizar la distribución de los ausentes**]{.hl-yellow}

```{r}
md.pattern(starwars_lm)
```
...
$\mathcal{H}_0$: valor más alto/bajo no es outlier

$\mathcal{H}_1$: valor más alto/bajo sí es outlier
...
Aprenderemos en la siguiente clase como imputar atípicos, de momento [**vamos a eliminarlos imputados según la media**]{.hl-yellow}


```{r}
starwars_sin_outliers <- 
  starwars_outliers_mean |> 
  drop_na()
starwars_sin_outliers
```
...
En este caso [**no tenemos altas correlaciones**]{.hl-yellow} entre las variables: nos interesa predictoras lo más incorreladas posibles entre sí y lo más correladas con la objetivo

```{r}
starwars_sin_outliers |> corrr::correlate()
```
