...
-   [**Evaluación**]{.hl-yellow}. A lo largo del curso se han planteado [**4 entregas individuales**]{.hl-purple} (se realizarán integramente en clase), así como una [**entrega final grupal**]{.hl-purple} (entre 2 y 4 personas) donde se deberá presentar el análisis realizado de un caso real.

. . .

-   [**Nota mínima**]{.hl-yellow}. Para no ir al examen final se [**deberá obtener al menos un 4/10**]{.hl-purple} en cada entrega o una [**media superior al 6/10**]{.hl-purple}. Se deberá obtener además en la [**entrega grupal una nota superior a 6-6.5-7/10**]{.hl-purple} (según 2-3-4 personas)
...
|   [5](#clase-5-importexport-comunicar)   | S3     | 25 sep |         Importar/exportar e intro a Quarto/rmd         | [📝]()  |                [💻](#tu-turno-4)                |         [🐣](#caso-práctico-3)         |         |     |     |
...
2.  ¿Cuántos [**registros tenemos de mayo**]{.hl-yellow}? ¿Y de abril?

3.  Construye una nueva variable `date` con la [**fecha**]{.hl-yellow} de cada registro

4.  Crea una nueva variable `temp_celsius` con la [**temperatura en ºC**]{.hl-yellow}

5.  ¿Cuál fue la [**media de temperatura**]{.hl-yellow} del mes de agosto?

6.  Extrae los días en los que la [**temperatura superó los 30 grados**]{.hl-yellow} y calcula la cantidad de días en los que lo hizo.
...
1.  ¿Qué [**representan**]{.hl-yellow} los datos?

2.  ¿Qué [**tipo de dato**]{.hl-yellow} es? En caso de que no lo sea, convierte a un `tibble` (renombra con `USArrests_tb`, por ejemplo)

3.  ¿Cuántos registros hay? ¿Y variables? ¿De qué tipo es cada una?

4.  Incorpora una [**variable extra con el nombre de cada estado**]{.hl-yellow} (piensa como obtenerlo del dataset original)

5.  ¿Dónde hay [**más asesinatos**]{.hl-yellow} (`murder`) en media: en los estados con población urbana superior al 75% o en los de menos del 55%?
...
En el paquete `{tidyr}` contamos con el datset `who`. Con la menor ayuda posible, debes ser capaz de manera autonónoma de

-   buscar la ayuda
-   entender las variables (y conocer sus nombres)
-   entender los tipos de variables
-   eliminar lo que sobra (redundante)
-   convertirlo a tidydata.
...
Por último, también podemos añadir pies de gráficas o imágenes, como veremos más adelante.

&nbsp;

Tienes toda la información detallada en **<https://quarto.org/>**
